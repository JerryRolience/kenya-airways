...
export function FlightList({ flights, searchParams, direction }: FlightListProps) {
  if (flights.length === 0) {
    return (
  ...
  return (
    <div className="space-y-3">
      {flights.map(flight => (
        <FlightCard key={flight.id} flight={flight} searchParams={searchParams} direction={direction} />
      ))}
    </div>
  )
}
