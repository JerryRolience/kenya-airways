...
// Mock data fetch — replace with actual hook
function useJobOpeningDetail(id: string | null) {
  // Replace with your actual data fetching hook
  // const { data, isLoading } = useFetchJobOpeningById(id);
...
export function JobOpeningDetailSheet({ open, onOpenChange, openingId, onEdit }: JobOpeningDetailSheetProps) {
  const { data: opening, isLoading } = useJobOpeningDetail(openingId)
      ...
      <SheetContent side="right" className="w-full sm:max-w-lg lg:max-w-xl overflow-y-auto p-0">
        {isLoading ? (
          ...
          <div className="p-6 text-center">
            ...
            <p className="mt-4 text-sm text-muted-foreground">Job opening not found</p>
          </div>
        )}
      </SheetContent>
    </Sheet>
