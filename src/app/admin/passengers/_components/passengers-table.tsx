...
import { useFetchPassengers } from "@/hooks/passenger/use-fetch-passengers"
import { usePassengersTableFilters } from "@/hooks/tables/use-passengers-table-filters"
...
import { passengerColumns } from "./passenger-columns"
import { PassengerTableFilter } from "./passenger-table-filters"
...
export function PassengersTable() {
  ...
  const { status: statusFilter, nationalities: nationalityFilters } = usePassengersTableFilters(columnFilters)
  ...
  const {
    ...
    hasUserAccount: statusFilter,
    nationality: nationalityFilters,
  })
  ...
  const passengers: PassengerListItem[] = response?.success ? (response.data?.data ?? []) : []
  ...
  const columns = useMemo(() => passengerColumns, [])
  ...
  const errorMessage = error instanceof AppError ? error.message : "We couldn't load the passenger list. Check your connection and try again."
    ...
    <Card className="w-full max-w-full overflow-hidden">
      <CardHeader className="pb-0 px-3 sm:px-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-base font-semibold">Passenger Registry</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {isLoading ? "Loading passenger..." : total > 0 ? `${total.toLocaleString()} passenger${total === 1 ? "" : "s"} registered` : "No passengers registered yet"}
            </p>
          </div>
        ...
        <PassengerTableFilter table={table} searchInput={searchInput} handleSearch={handleSearch} isFiltered={isFiltered} handleReset={handleReset} refetch={refetch} isFetching={isFetching} />
      </CardHeader>
          ...
          <div className="relative w-full overflow-x-auto">
            <div className="max-w-120 lg:min-w-full">
              <Table className="w-full table-auto">
                ...
                {isLoading ? (
                  <DataTableLoadingState columns={passengerColumns} rows={10} />
                ) : isError ? (
                  <TableBody>
                    <TableRow>
                      <TableCell colSpan={passengerColumns.length}>
                  ...
                  <TableBody>
                    <TableRow>
                      <TableCell colSpan={passengerColumns.length}>
                        <DataTableEmptyState
                          icon={Users}
                          title={isFiltered ? "No matching passengers" : "No passengers yet"}
                          description={isFiltered ? "Try adjusting your search or clearing your filters." : "Register your first passenger using the Add Passenger button above."}
                        />
                      </TableCell>
                    </TableRow>
                  </TableBody>
