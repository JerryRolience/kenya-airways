...
import { useFetchJobOpenings } from "@/hooks/job/use-fetch-job-openings"
import { useJobOpeningTableFilters } from "@/hooks/tables/use-job-opening-table-filters"
...
import { jobOpeningColumns } from "./job-opening-columns"
import { JobOpeningTableFilter } from "./job-opening-table-filter"
...
export function JobOpeningsTable() {
  ...
  const { status: statusFilter } = useJobOpeningTableFilters(columnFilters)
  ...
  const openings: JobOpeningListItem[] = response?.success ? (response.data?.data ?? []) : []
  ...
  const columns = useMemo(() => jobOpeningColumns, [])
  ...
  const errorMessage = error instanceof AppError ? error.message : "We couldn't load the job openings. Check your connection and try again."
  ...
  return (
    <Card className="w-full max-w-full overflow-hidden">
      <CardHeader className="pb-0 px-3 sm:px-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {isLoading ? "Loading job openings..." : total > 0 ? `${total.toLocaleString()} job opening${total === 1 ? "" : "s"} available` : "No job openings available yet"}
            </p>
          </div>
        ...
        <JobOpeningTableFilter table={table} searchInput={searchInput} handleSearch={handleSearch} isFiltered={isFiltered} handleReset={handleReset} refetch={refetch} isFetching={isFetching} />
      </CardHeader>
          ...
          <div className="relative w-full overflow-x-auto">
            <div className="max-w-120 lg:min-w-full">
              <Table className="w-full table-auto">
                ...
                {isLoading ? (
                  <DataTableLoadingState columns={jobOpeningColumns} rows={10} />
                ) : isError ? (
                  <TableBody>
                    <TableRow>
                      <TableCell colSpan={jobOpeningColumns.length}>
                  ...
                  <TableBody>
                    <TableRow>
                      <TableCell colSpan={jobOpeningColumns.length}>
                        <DataTableEmptyState
                          icon={Users}
                          title={isFiltered ? "No matching job openings" : "No job openings yet"}
                          description={isFiltered ? "Try adjusting your search or clearing your filters." : "Create your first job opening using the Add Job Opening button above."}
                        />
                      </TableCell>
                    </TableRow>
                  </TableBody>
