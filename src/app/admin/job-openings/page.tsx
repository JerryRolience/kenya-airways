...
import { useFetchJobOpeningStats } from "@/hooks/job/use-fetch-job-opening-stats"
...
export default function JobOpeningsPage() {
  ...
  const { isLoading: statsLoading, statsData, isError, refetch, error, isFetching: statsFetching } = useFetchJobOpeningStats()
    ...
    <div className="space-y-4 sm:space-y-6 px-3 sm:px-4 py-3 sm:py-4 w-full min-w-0">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight truncate">Job Openings</h1>
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">Manage open positions and review applications</p>
        </div>
