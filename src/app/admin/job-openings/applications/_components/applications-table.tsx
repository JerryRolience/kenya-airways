...
import { useFetchJobApplications } from "@/hooks/job/use-fetch-job-applications"
import { useApplicationTableFilters } from "@/hooks/tables/use-application-table"
...
export function ApplicationsTable() {
  ...
  const { status: statusFilter } = useApplicationTableFilters(columnFilters)
  ...
  const applications: ApplicationListItem[] = response?.success ? (response.data?.data ?? []) : []
  ...
  const errorMessage = error instanceof AppError ? error.message : "We couldn't load the application list. Check your connection and try again."
    ...
    <Card className="w-full max-w-full overflow-hidden">
      <CardHeader className="pb-0 px-3 sm:px-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-base font-semibold">Application Registry</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {isLoading ? "Loading applications..." : total > 0 ? `${total.toLocaleString()} application${total === 1 ? "" : "s"} registered` : "No applications registered yet"}
            </p>
          </div>
        ...
        <ApplicationTableFilter table={table} searchInput={searchInput} handleSearch={handleSearch} isFiltered={isFiltered} handleReset={handleReset} refetch={refetch} isFetching={isFetching} />
      </CardHeader>
          ...
          <div className="relative w-full overflow-x-auto">
            <div className="max-w-120 lg:min-w-full">
              <Table className="w-full table-auto">
                ...
                {isLoading ? (
                  <DataTableLoadingState columns={applicationColumns} rows={10} />
                ) : isError ? (
                  <TableBody>
                    <TableRow>
                      <TableCell colSpan={applicationColumns.length}>
                    ...
                    <TableRow>
                      <TableCell colSpan={applicationColumns.length}>
                        <DataTableEmptyState
                          ...
                          description={isFiltered ? "Try adjusting your search or clearing your filters." : "Register your first application using the Add Application button above."}
                        />
                      </TableCell>
                    </TableRow>
                  </TableBody>
