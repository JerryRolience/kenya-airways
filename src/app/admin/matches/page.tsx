...
import { useFetchMatchStats } from "@/hooks/matches/use-fetch-match-stats"
import { Plus, RefreshCw } from "lucide-react"
import { useState } from "react"
import { MatchesTable } from "./_component/matches-table"
import { MatchForm } from "@/components/forms/match-form"

export default function MatchesPage() {
  const [addOpen, setAddOpen] = useState(false)

  const { isLoading: statsLoading, statsData, isError, refetch, error, isFetching: statsFetching } = useFetchMatchStats()
    ...
    <div className="space-y-4 sm:space-y-6 px-3 sm:px-4 py-3 sm:py-4 w-full min-w-0">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight truncate">Employee Matches</h1>
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">Match employees to job openings and manage assignments</p>
        </div>
      ...
      {/* Match creation form dialog */}
      <MatchForm open={addOpen} setOpen={setAddOpen} />
    </div>
  )
}
