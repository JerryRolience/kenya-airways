...
import { useFetchMatches } from "@/hooks/matches/use-fetch-matches"
import { useMatchTableFilters } from "@/hooks/tables/use-match-table-filters"
...
import { MatchTableFilter } from "./match-table-filters"
...
export function MatchesTable() {
  ...
  const { status: statusFilter } = useMatchTableFilters(columnFilters)
  ...
  const matches: MatchListItem[] = response?.success ? (response.data?.data ?? []) : []
  ...
  const columns = useMemo(() => matchColumns, [])
  ...
  const errorMessage = error instanceof AppError ? error.message : "We couldn't load the match list. Check your connection and try again."
    ...
    <Card className="w-full max-w-full overflow-hidden">
      <CardHeader className="pb-0 px-3 sm:px-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-base font-semibold">Match Registry</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {isLoading ? "Loading match..." : total > 0 ? `${total.toLocaleString()} match${total === 1 ? "" : "es"} registered` : "No matches registered yet"}
            </p>
          </div>
        ...
        <MatchTableFilter table={table} searchInput={searchInput} handleSearch={handleSearch} isFiltered={isFiltered} handleReset={handleReset} refetch={refetch} isFetching={isFetching} />
      </CardHeader>
          ...
          <div className="relative w-full overflow-x-auto">
            <div className="max-w-120 lg:min-w-full">
              <Table className="w-full table-auto">
                ...
                {isLoading ? (
                  <DataTableLoadingState columns={matchColumns} rows={10} />
                ) : isError ? (
                  <TableBody>
                    <TableRow>
                      <TableCell colSpan={matchColumns.length}>
                  ...
                  <TableBody>
                    <TableRow>
                      <TableCell colSpan={matchColumns.length}>
                        <DataTableEmptyState
                          icon={Users}
                          title={isFiltered ? "No matching matches" : "No matches yet"}
                          description={isFiltered ? "Try adjusting your search or clearing your filters." : "Register your first match using the Add Match button above."}
                        />
                      </TableCell>
                    </TableRow>
                  </TableBody>
