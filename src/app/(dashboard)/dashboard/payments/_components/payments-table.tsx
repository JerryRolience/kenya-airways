...
import { useFetchUserPayments } from "@/hooks/payment/use-fetch-user-payments"
import { usePaymentsTableFilters } from "@/hooks/tables/use-payments-table"
...
import { PaymentTableFilter } from "./payment-table-filters"
...
export function PaymentsTable() {
  ...
  const { methods: paymentMethods } = usePaymentsTableFilters(columnFilters)
  ...
  useEffect(() => {
    resetCursor()
  }, [debouncedSearch, paymentMethods, resetCursor])

  const payments: UserPayment[] = response?.success ? (response.data?.data ?? []) : []
  ...
  const errorMessage = error instanceof AppError ? error.message : "We couldn't load the payment list. Check your connection and try again."
    ...
    <Card className="w-full max-w-full overflow-hidden">
      <CardHeader className="pb-0 px-3 sm:px-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-base font-semibold">Payment Registry</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {isLoading ? "Loading payment..." : total > 0 ? `${total.toLocaleString()} payment${total === 1 ? "" : "s"} registered` : "No payments registered yet"}
            </p>
          </div>
        ...
        <PaymentTableFilter table={table} searchInput={searchInput} handleSearch={handleSearch} isFiltered={isFiltered} handleReset={handleReset} refetch={refetch} isFetching={isFetching} />
      </CardHeader>
          ...
          <div className="relative w-full overflow-x-auto">
            <div className="max-w-120 lg:min-w-full">
              <Table className="w-full table-auto">
                ...
                {isLoading ? (
                  <DataTableLoadingState columns={paymentColumns} rows={10} />
                ) : isError ? (
                  <TableBody>
                    <TableRow>
                      <TableCell colSpan={paymentColumns.length}>
                    ...
                    <TableRow>
                      <TableCell colSpan={paymentColumns.length}>
                        <DataTableEmptyState
                          ...
                          description={isFiltered ? "Try adjusting your search or clearing your filters." : "Register your first payment using the Add Payment button above."}
                        />
                      </TableCell>
                    </TableRow>
                  </TableBody>
