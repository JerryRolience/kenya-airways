...
import { useFetchUserBookings } from "@/hooks/booking/use-fetch-user-bookings"
import { useBookingsTableFilters } from "@/hooks/tables/use-bookings-table-filters"
...
import { BookingTableFilter } from "./booking-table-filters"
...
export function BookingsTable() {
  ...
  const { status: statusFilter, paymentStatuses } = useBookingsTableFilters(columnFilters)
  ...
  useEffect(() => {
    resetCursor()
  }, [debouncedSearch, statusFilter, paymentStatuses, resetCursor])

  const bookings: BookingListItem[] = response?.success ? (response.data?.data ?? []) : []
  ...
  const errorMessage = error instanceof AppError ? error.message : "We couldn't load the booking list. Check your connection and try again."
    ...
    <Card className="w-full max-w-full overflow-hidden">
      <CardHeader className="pb-0 px-3 sm:px-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-base font-semibold">Booking Registry</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {isLoading ? "Loading booking..." : total > 0 ? `${total.toLocaleString()} booking${total === 1 ? "" : "s"} registered` : "No bookings registered yet"}
            </p>
          </div>
        ...
        <BookingTableFilter table={table} searchInput={searchInput} handleSearch={handleSearch} isFiltered={isFiltered} handleReset={handleReset} refetch={refetch} isFetching={isFetching} />
      </CardHeader>
          ...
          <div className="relative w-full overflow-x-auto">
            <div className="max-w-120 lg:min-w-full">
              <Table className="w-full table-auto">
                ...
                {isLoading ? (
                  <DataTableLoadingState columns={bookingColumns} rows={10} />
                ) : isError ? (
                  <TableBody>
                    <TableRow>
                      <TableCell colSpan={bookingColumns.length}>
                  ...
                  <TableBody>
                    <TableRow>
                      <TableCell colSpan={bookingColumns.length}>
                        <DataTableEmptyState
                          icon={Users}
                          title={isFiltered ? "No matching bookings" : "No bookings yet"}
                          description={isFiltered ? "Try adjusting your search or clearing your filters." : "Register your first booking using the Add Booking button above."}
                        />
                      </TableCell>
                    </TableRow>
                  </TableBody>
